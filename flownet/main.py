...
if __name__ == '__main__':
    ...
    # "FlowNet2" is a class name in module "models.py" and same as others
    # "$default" is set as default value of args --"$argument_for_class"
    # for example --model 'FlowNet2C' ...
    # if --model is not given, it will be set to 'FlowNet2' hear
    # parameter for initialize class set by --"$argument_for_class" can be given by command line --"$argument_for_class"_"$para_name"
    # for example if --model is not given and class "FlowNet2" has parameter named abc, then it can be set as --model_abc "value of abc"
    # then the "value of abc" will be forward as parameter "abc" to initialize object of class "FlowNet2C"
    ...
    tools.add_arguments_for_module(parser, datasets, argument_for_class='inference_dataset', default='MpiSintelClean', 
                                    skip_params=['is_cropped',"camera"],
                                    parameter_defaults={'root': './MPI-Sintel/flow/training',
                                                        'replicates': 1})


    # change workdir to current direction
    ...
    with tools.TimerBlock("Parsing Arguments") as block:
        ...
        # a trick to get all args with default value: set a "ignore" args then call parser.add_argument by set this "ignore" only
        # then the parser will return args object with only args with default value
        parser.add_argument('--IGNORE',  action='store_true')
        defaults = vars(parser.parse_args(['--IGNORE']))

        # Print all arguments, color the arg with non-defaults value
        for argument, value in sorted(vars(args).items()):
            # colorrama is a module for printing color text on console
            reset = colorama.Style.RESET_ALL
            color = reset if value == defaults[argument] else colorama.Fore.MAGENTA
            block.log('{}{}: {}{}'.format(color, argument, value, reset))

        # temporally ignore the fp16
        args.fp16 = None
        ...
        if args.inference_dataset_duration <= 0:
    ...
    #学习，troch的dataset类和dataloader类
    with tools.TimerBlock("Initializing Datasets") as block:
        args.effective_batch_size = args.batch_size * args.number_gpus
        args.effective_inference_batch_size = args.inference_batch_size * args.number_gpus if not args.show else 1
        ...
        if exists(args.inference_dataset_root) or args.camera:
            inference_dataset = args.inference_dataset_class(args, False,camera=args.camera, **tools.kwargs_from_args(args, 'inference_dataset'))
            ...
            # if args.inference_dataset == "VideoFile":
            #     FPS = int(inference_dataset.fps)
            #     FRAME_SIZE = args.inference_size
            # ## print("inferenceSize:{}".format(FRAME_SIZE))
    ...
    with tools.TimerBlock("Building {} model".format(args.model)) as block:
        ...
        #inner class for aggregation of module and loss
        #学习，如果构建自己的torch模型
        class ModelAndLoss(nn.Module):
            ...
            def forward(self, data, target, inference=False):
                ...
                # only return loss in training/validation
                if not inference :
                    return loss_values
                else:
        ...
        if args.cuda and (args.number_gpus > 0) and args.fp16:
            ...
            #param_copy = [param.clone().type(torch.cuda.FloatTensor) for param in model_and_loss.parameters()]
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            if not is_validate and args.fp16:
                ...
                for i in range(len(params)):
                   ...
                   param_copy[i].grad.mul_(1./args.fp16_scale)
                optimizer.step()
                for i in range(len(params)):
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        if args.save_video and args.inference_dataset == "VideoFile":
            video_folder = "{}/inference/video/".format(args.save)
            #ori_frames= [cv2.cvtColor(data_tem[idx].transpose(1, 2, 3, 0)[0],cv2.COLOR_RGB2BGR) for idx in range(data_tem.shape[0])]
            ori_cap = cv2.VideoCapture(args.inference_dataset_root)
            ori_cap.set(cv2.CAP_PROP_POS_FRAMES,inference_dataset.start_frame)
            if not os.path.exists(video_folder):
                os.makedirs(video_folder)
            filename = "Flow_vis_" + os.path.split(args.inference_dataset_root)[-1] if not args.camera else "Flow_vis_camera.avi"
            full_filename = os.path.join(video_folder,filename)

            v_writer = cv2.VideoWriter(full_filename,
                    apiPreference=cv2.CAP_ANY,
                    fourcc=cv2.VideoWriter_fourcc(*'MJPG'),
                    fps=int(inference_dataset.fps),
                    frameSize=(inference_dataset.render_size[1]*2,inference_dataset.render_size[0]))
        ...
        for batch_idx, (data, target) in enumerate(progress):
            #data:[batch_idx,channel,2,row_index,col_idx]


            if args.cuda:
                # if args.fp16:
                #     data, target = [d.cuda(async=True).half() for d in data], [t.cuda(async=True).half() for t in target]
                # else:
                #data, target = [d.cuda(async=True) for d in data], [t.cuda(async=True) for t in target]
                data = data.cuda(async=True)
                target = target.cuda(async=True)
            ...
            print()
            s = time.time()
            print("other_time:{}".format(s - e))
            with torch.no_grad():
                losses, output = model(data, target, inference=True)
            e = time.time()
            print()
            print()
            print("forward_time:{}".format(e-s))
            losses = [torch.mean(loss_value) for loss_value in losses]
            loss_val = losses[0] # Collect first loss for weight update
            ...
            if args.show:
                ...
                flows = o[0].transpose(1, 2, 0)
                frames = flowlib.flow_to_image(flows)
                cv2.imshow("test_opticla_flow",frames)
                if cv2.waitKey(1):
                    break
            # save flow
            if args.save_flow or args.render_validation:
                for i in range(args.effective_inference_batch_size):
                    ...
                    flow_utils.writeFlow( join(flow_folder, '%08d.flo'%(batch_idx * args.effective_inference_batch_size + i)),  _pflow)
            # change flow to image and save video
            if args.save_video and args.inference_dataset == "VideoFile":
                ori_frames = [ori_cap.read()[1] for _ in range(args.effective_inference_batch_size)]
                o = output.data.cpu().numpy()
                # transpose channel to list demension
                flows = [o[i].transpose(1, 2, 0) for i in range(args.effective_inference_batch_size)]
                frames = [flowlib.flow_to_image(f) for f in flows]
                # print("{}~~{}".format(str(len(frames)),len(ori_frames)))
                # print("{}~~{}".format(frames[0].shape,ori_frames[0].shape))
                assert (len(frames) == len(ori_frames) and frames[0].shape == ori_frames[0].shape)
                frames = [np.concatenate((frames[idx],ori_frames[idx]),axis=1) for idx in range(len(frames)) ]
                # print(frames[0].shape)
                # print("saved_size:{}".format(frames[0].shape))
                for f in frames:
                    v_writer.write(f)
    ...
    for epoch in progress:
        ...
        if not args.skip_validation and ((epoch - 1) % args.validation_frequency) == 0:
            ...
            # save checkpoint once validation
