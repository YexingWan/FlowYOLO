...
:arg
    module: module
    exclude: list of object
:return
    dict
    
pack all class of a module(a file or a module) in a dict {class_name: class}
...
# built an arg_group for different modules
def add_arguments_for_module(parser:argparse.ArgumentParser,
                             module,
                             argument_for_class:str,
                             default:str,
                             skip_params:list=[],
    ...
    # built a arg_group by key name and default by default
    ...
    #get class object from module_dict by arg:argument_for_class
    class_obj = module_dict[vars(args)[argument_for_class]]

    # get specific args for class initialization
    argspec = inspect.getfullargspec(class_obj.__init__)

    #都倒过来，就能一一对应上了，不然不好处理
    ...
    for i, arg in enumerate(args):
        ...
        if arg not in skip_params + ['self', 'args']:
            #如果当前class的这个arg的设定有根据parameter_defaults提供default值
            ...
            #或者如果class的初始化函数init本身对这个arg提供default
            ...
            #如果两者都不是，那就是正常的positional args
            else:
                print(("[Warning]: non-default argument '{}' detected on class '{}', set in parameter_defaults as a dict"
